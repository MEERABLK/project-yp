...
class ApiService {
  Future<List<YugiohModel>?> getYugioh() async {
  ...
  Future<List<PokemonModel>?> getPokemon() async {
    ...
    {
      // Combine the base URL and endpoint (e.g. "https://.../users")
      var url = Uri.parse("https://play.pokemonshowdown.com/data/pokedex.json");
      //Send the GET request
      var response = await http.get(url);
      if (response.statusCode == 200) {
        // If status code is 200 (OK), decode the JSON into a list of UserModel
        List<PokemonModel> _model = pokemonModelFromJson(response.body);
